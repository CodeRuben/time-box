...
- `book-log-list` shows the **Books** heading and status-grouped tables or empty state.
...
- Sign in, then open **Books** from the left sidebar (desktop) or the mobile **Open menu** sheet.
...
- **Open list.** Navigate to `<baseUrl>/reading-journal` or click link **Books**. Heading **Books** and subtitle *Track what you read, day by day.* are visible.
...
- **Proof.** Snapshot `artifacts/<RUN_ID>/book-log/list.aria.txt` and screenshot `artifacts/<RUN_ID>/book-log/list.png`. Artifacts show **Books** and `Verify Book` in the table.

## Gotchas

- Books is signed-in only; guests do not see the nav link. Direct `/reading-journal` redirects to another allowed page (usually Planner). A **Sign in** button appears only on the **No pages available** fallback card when no other nav destination exists.
